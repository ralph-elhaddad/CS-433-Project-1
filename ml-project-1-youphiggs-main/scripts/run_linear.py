...
if __name__ == "__main__":
    ...
    print("\n>>> feature expansion")
    # polynomial expansion up to degree of 10
    tX_poly = build_poly(tX, 10)
    tX_poly = add_shift(tX_poly)
    tX_test_poly = build_poly(tX_test, 10)
    ...
    tr_loss, te_loss, accr = cv_eval_notrain(y, tX_poly, w_opt, k_fold=5, threshold=0, seed=1, islinear=True)
    print("cross validation result:\n mean train loss {:.2e}\t mean test loss {:.2e}\t mean acccuracy {:.2%}".format(tr_loss, te_loss, accr))

    print("\n>>> generating prediction")
    OUTPUT_PATH = './pred_best_linear.csv'
    y_pred = pred_linear(tX_test_poly, w_opt, threshold=0)
