...
def linear_sgd(y, x, max_iters=100, maxstep=0.5, gamma=0.1, batch_size= 1):
    ...
    min_loss = np.inf
    w_opt = w_initial
    for batch_y, batch_x in batch_iter(y, tX_poly, batch_size, num_batches=max_iters):
        grad = compute_gradient(batch_y, batch_x, w)
...
def logistic_regression_newton(y, tx, max_iter=100, threshold=1e-8, gamma=1.):
    """
...
def logstic_regression(y, tx, max_iter=300, gamma=.1, pred_threshold=0.5, conv_threshold=1e-6, isplot=False, maxstep=0.5):
    """
    logistic regression with gradient descent
   ...
   # init parameters
    ...
    for iter in range(max_iter):
        ...
        if iter % 100 == 0:
            print("Current iteration={i}, regularized loss: {ls_reg}".format(i=iter, ls_reg=ls))
...
def logistic_regression_penalized_gradient_descent(y, tx, max_iter=300, gamma=.1, lambda_=0.1, pred_threshold=0.5, conv_threshold=1e-6, isplot=False, maxstep=0.5):
    """
    ...
    for iter in range(max_iter):
        ...
        if isplot:
            ...
            accrs.append([iter, get_accuracy(pred_logistic(tx, w, pred_threshold), y)])
        if iter % 100 == 0:
            print("Current iteration={i}, regularized loss: {ls_reg}".format(i=iter, ls_reg=ls_reg))
