...
if __name__ == "__main__":
    ...
    print(
        "{} data points, {} features in train set".format(
            tX_poly.shape[0], tX_poly.shape[1]
        )
    )
    print(
    ...
    tr_loss, te_loss, accr = cv_eval_notrain(
        y, tX_poly, w_opt, 5, 0.5, seed=1, islinear=False
    )
    print(
        "cross validation result:\n mean train loss {:.2e}\t mean test loss {:.2e}\t mean acccuracy {:.2%}".format(
            tr_loss, te_loss, accr
        )
    )
